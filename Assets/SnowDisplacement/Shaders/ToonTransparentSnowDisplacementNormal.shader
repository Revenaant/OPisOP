...
Shader "Toon/Lit Snow Buildup Normal Transparent" 
{
	Properties
	{
...
#pragma lighting ToonRamp exclude_path:prepass
		...
		float4 direction()
		{
			return mul(_SnowAngle, unity_WorldToObject);
		}
		...
		void disp(inout appdata_full v, out Input o)
		{
			...
			float4 snowC = direction(); // snow direction convertion to worldspace
			//float4 snowC = mul(_SnowAngle , unity_WorldToObject); // snow direction convertion to worldspace
			//float4 snowC = mul(_SnowAngle , unity_ObjectToWorld); // snow direction convertion to worldspace
			//float4 snowC = _SnowAngle; // snow direction convertion to worldspace
		...
		void surf(Input IN, inout SurfaceOutput o) 
		{
			...
			float3 baseColor = c.rgb * _Color;
			o.Albedo = baseColor;// base color
								 //clip(c.rgb);
			half rim = 1.0 - saturate(dot(normalize(IN.viewDir), o.Normal)); // rimlight
			if (dot(o.Normal, snowC) >= _SnowSize - 0.4) { // if dot product result is higher than snow amount, we turn it into snow
			...
			if ((o.Albedo == baseColor).r) discard;
			else o.Alpha = c.a;
		}
		ENDCG
	}
