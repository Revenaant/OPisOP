...
public class UIParticleSystem : MonoBehaviour {
    ...
    {
        StopAllCoroutines();
        foreach (Image image in ParticlePool)
    ...
    private void Init()
    {
        ...
        for (int i = 0; i < ParticlePool.Length; i++)
        {
            ...
            //StartCoroutine(MyCoroutines.WaitOneFrame(() => gameObject.layer = Layer.value));
            //gameObject.layer = LayerMask.NameToLayer("Untouchable UI");
            gameObject.layer = Layer.LayerIndex;
        }
    }
